...
# Labels positioned in the middle of the line
for _, row in highlighted.iterrows():
    ...
    # Custom vertical offsets for each country
    if row['country'] == 'Saudi Arabia':
        ...
        # Middle of line, below replacement level (2.1)
        label_x = mid_x
        label_y = 1.95  # Below 2.1
        xytext = (0, 0)
        ha = 'center'
    elif row['country'] == 'Korea (Republic of)':
        # Middle of line, below 1.0 grid line
        label_x = mid_x
        label_y = 0.85  # Below 1.0
        xytext = (0, 0)
        ha = 'center'
    elif row['country'] == 'Qatar':
        # Middle of line, above green line (2.1)
        label_x = mid_x
        label_y = 2.25  # Above 2.1
        xytext = (0, 0)
        ha = 'center'
    else:
        label_x = row['gni_now']
...
legend_bbox_transformed = legend_bbox.transformed(ax.transAxes.inverted())

# Statistics box 0.1cm (approximately 0.01 in figure coordinates) BELOW legend
stats_y_position = legend_bbox_transformed.y0 - 0.015
...
ax.text(
    0.98, stats_y_position, stats_text,
...
print("\n" + "="*90)
print("=== RANKING BY CURRENT FERTILITY RATE (HIGHEST TO LOWEST) ===")
print("="*90)

# Sort by current fertility rate (HIGHEST to LOWEST)
current_fertility_ranking = results_df.sort_values('most_recent_fertility', ascending=False).copy()
...
top_10_lowest = current_fertility_ranking.tail(10).sort_values('most_recent_fertility')
...
# ========================================
# COMPREHENSIVE FERTILITY RANKINGS
# ========================================

print("\n" + "="*100)
print("=== RANKING BY TRANSITION YEAR (EARLIEST TO LATEST) ===")
print("="*100)

# Sort by transition year
transition_year_ranking = results_df.sort_values('transition_year', ascending=True).copy()
transition_year_ranking['trans_rank'] = range(1, len(transition_year_ranking) + 1)

print("\n{:<5} {:<35} {:<12} {:<12} {:<12} {:<12}".format(
    "Rank", "Country", "Trans Year", "Before TFR", "Current TFR", "Change"
))
print("-" * 100)

for _, row in transition_year_ranking.iterrows():
    # Highlight the 4 special countries
    if row['country'] in ['Saudi Arabia', 'Kuwait', 'Korea (Republic of)', 'Qatar']:
        marker = " ★"
    else:
    ...
    print("{:<5} {:<35} {:<12} {:<12.2f} {:<12.2f} {:<12.2f}{}".format(
        int(row['trans_rank']),
        row['country'][:34],
        row['transition_year'],
        row['mean_fertility_before'],
...
# Export
transition_year_ranking[['trans_rank', 'country', 'transition_year', 
                         'mean_fertility_before', 'most_recent_fertility', 
                         'fertility_change']].to_csv(
    'ranking_by_transition_year.csv', index=False
)
print("\n>>> Saved to 'ranking_by_transition_year.csv'")
...
current_fertility_ranking[['current_rank', 'country', 'transition_year', 
                           'mean_fertility_before', 'most_recent_fertility', 
                           'fertility_change']].to_csv(
    'ranking_by_current_fertility.csv', index=False
)
print("\n>>> Saved to 'ranking_by_current_fertility.csv'")

print("\n" + "="*100)
print("=== RANKING BY FERTILITY BEFORE DEVELOPED STATUS (HIGHEST TO LOWEST) ===")
print("="*100)

# Sort by fertility before transition (HIGHEST to LOWEST)
before_fertility_ranking = results_df.sort_values('mean_fertility_before', ascending=False).copy()
before_fertility_ranking['before_rank'] = range(1, len(before_fertility_ranking) + 1)
...
for _, row in before_fertility_ranking.iterrows():
    # Highlight the 4 special countries
    if row['country'] in ['Saudi Arabia', 'Kuwait', 'Korea (Republic of)', 'Qatar']:
        marker = " ★"
    else:
...
# Export
before_fertility_ranking[['before_rank', 'country', 'transition_year', 
                          'mean_fertility_before', 'most_recent_fertility', 
                          'fertility_change']].to_csv(
    'ranking_by_before_fertility.csv', index=False
)
print("\n>>> Saved to 'ranking_by_before_fertility.csv'")

print("\n" + "="*100)
print("=== SUMMARY: HIGHLIGHTED COUNTRIES (SAUDI ARABIA, KUWAIT, SOUTH KOREA, QATAR) ===")
print("="*100)

highlight_summary = results_df[results_df['country'].isin(['Saudi Arabia', 'Kuwait', 'Korea (Republic of)', 'Qatar'])].copy()

if len(highlight_summary) > 0:
    print("\n{:<25} {:<15} {:<15} {:<15} {:<15}".format(
        "Country", "Trans Year", "Before TFR", "Current TFR", "Change"
    ))
    print("-" * 100)
    
    for _, row in highlight_summary.iterrows():
        # Get ranks
        trans_rank = transition_year_ranking[transition_year_ranking['country'] == row['country']]['trans_rank'].values[0]
        current_rank = current_fertility_ranking[current_fertility_ranking['country'] == row['country']]['current_rank'].values[0]
        before_rank = before_fertility_ranking[before_fertility_ranking['country'] == row['country']]['before_rank'].values[0]
        
        print("{:<25} {:<15} {:<15.2f} {:<15.2f} {:<15.2f}".format(
            row['country'].replace('Korea (Republic of)', 'South Korea'),
            row['transition_year'],
            row['mean_fertility_before'],
            row['most_recent_fertility'],
        ...
        print(f"  → Ranks: Trans Year #{int(trans_rank)}, Current TFR #{int(current_rank)}, Before TFR #{int(before_rank)}")
        print()

print("\n" + "="*100)
print("=== ALL RANKING ANALYSES COMPLETE ===")
print("="*100)
print("\nGenerated CSV files:")
print("  1. ranking_by_transition_year.csv")
print("  2. ranking_by_current_fertility.csv")
print("  3. ranking_by_before_fertility.csv")
print("\n★ = Highlighted countries (Saudi Arabia, Kuwait, South Korea, Qatar)")
print("="*100)